...
```
Der durchschnittliche marginale Effekt einer Veranderung der Variable retire ist 0.04723126: im Mittel uber alle Personen haben wir einen positiven Effekt von Renter-Status einer Person von 0.04723126 der Wahrscheinlichkeit, dass die Person krankversichert ist.
...
Der marginale Effekt im Mittelwert bezuglich der Variable educyear betragt 0.018 Positiver Effekt  eines weiteren Bildungsjahres auf die Wahrscheinlichkeit, krankversichert zu sein. Jedes weitere Bildungsjahr erhoht die Wahrscheinlichkit um 0.018.
...
```{r}
logRegMax <- glm(ins ~ married + retire + hstatusg + loghhincome + 
                   age + hisp + educyear + married^2 + retire^2 + hstatusg^2 + loghhincome^2 + 
                   age^2 + hisp^2 + educyear^2, family = binomial(link = "logit"), data = insur)
stepAIC(logRegMax, direction = "backward")
```
Schrittweise Selektion (AIC): das Maximalmodell als Ausgangsmodell: $ins = β_0 + β_1*married + β_2*retire + β_3*hstatusg + β_4*loghhincome + β_5*age + β_6*hisp +β_7*educyear + β_8*married^2 + β_9*retire^2 + β_10*hstatusg^2 + β_11*loghhincome^2 + β_12*age^2 + β_13*hisp^2 +β_14*educyear^2$. 
...
```{r}
marg_table_insr_prob <- prop.table(table(Obs = insur$ins, Pred = round(fitted(probRegFull))))
marg_table_insr_prob
marg_table_insr_prob[1, 1] + marg_table_insr_prob[2, 2]
```
...
```{r}
summary(margins(logRegFull, type = "response"))
summary(margins(probRegFull, type = "response"))
```
Der durchschnittliche marginale Effekt einer Veranderung der Variable retire im Probit-Modell betragt 0.0458, wahrend der durchschnittliche marginale Effekt einer Veranderung der Variable retire im Logit-Modell ist 0.0471.
