...
```{r}
linRegFull <- lm(cons ~ inc + factor(large) + factor(south) + factor(female) + factor(mar), data = shiw2012)
summary(linRegFull)

linRegElas <- lm(log(cons) ~ log(inc) + factor(large) + factor(south) + factor(female) + factor(mar), data = shiw2012)
summary(linRegElas)

```
Bei einer Erhohung des Einkommens um ein Prozent erhohen sich die Konsumausgaben c.p. um 0.54%.
...
```{r}
100*coef(linRegElas)["factor(female)1"]
100*(exp(coef(linRegElas)["factor(female)1"]) - 1)
```
Wenn Haushaltsvorstand weiblich ist, sinken die Konsumausgaben c.p. um 2.68% (näherungsweise um 2.71%)
...
```{r}
s2 <- summary(linRegElas)$sigma^2
kf <- exp(s2/2)

pred_elas <-predict(linRegElas, data.frame(inc = 30000, large = 1, south = 0, female = 1, mar = 0))

kf*exp(pred_elas)


```
Ein Haushalt, der groß ist,  uber ein Einkommen von 30000 EUR verfuge,  einen unverheirateten und weiblichen Haushaltsvorstand habe und sich im Norden des Landes beﬁnde, gibt 23917.97  EUR fur Konsum aus.
